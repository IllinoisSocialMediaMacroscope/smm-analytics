...
class Classification:
    ...
    def split(self, ratio):
        ...
        div_split = plot([trace], output_type='div', image='png',
                         auto_open=False, image_filename='plot_img')
        ...
        try:
            with open(self.localSavePath + fname1, 'w', newline="",
                      encoding='utf-8') as f:
                writer = csv.writer(f)
        ...
        try:
            with open(self.localSavePath + fname2, 'w', newline="",
                      encoding='utf-8') as f:
                writer = csv.writer(f)
        ...
        return {'div': div_url, 'training': training_url,
...
def lambda_handler(event, context):
    ...
    # arranging the paths
    awsPath = event['s3FolderName'] + '/ML/classification/' + event[
        'uid'] + '/'
    localSavePath = '/tmp/' + event['s3FolderName'] + '/ML/classification/' + \
                    event['uid'] + '/'
