...
class Network:
    ...
    def __init__(self, awsPath, localSavePath, localReadPath,
        ...
        # corresponding to the JOB id to avoid overwrite
        ...
        if relationships == 'reply_to':
            if remoteReadPath.find('twitter-Tweet') != -1:
                ...
                df['reply_to'] = df['_source.text'].str.extract('^@([A-Za-z0-9-_]+)',
                                                                expand=True)
    ...
    def export_JSON(self):
        ...
        d3js_graph['nodes'] = [{'id': node['id'],
          'connectivity': self.graph.in_degree()[node['id']]
          + self.graph.out_degree()[node['id']]}
            for node in d3js_graph['nodes']]
        fname_d3js = 'd3js.json'
        with open(os.path.join(self.localSavePath, fname_d3js), "w") as f:
    ...
    def export_Gephi(self):
        ...
        nx.write_gml(self.graph,
                     os.path.join(self.localSavePath, fname_gephi))
    ...
    def export_Pajek(self):
        ...
        nx.write_pajek(self.graph,
                       os.path.join(self.localSavePath, fname_pajek))
    ...
    def draw_graph(self, relationships, layout):
        ...
        if relationships == 'reply_to':
            for node in self.graph.nodes():
                node_trace['marker']['color'].append(self.graph.in_degree()[node]
                                                     + self.graph.out_degree()[node])
                node_trace['text'].append("@" + node + " is replied by "
                                          + str(self.graph.in_degree()[node])
                                          + " user(s), and replies to "
                                          + str(self.graph.out_degree()[node]) + " user(s)")
            ...
            for node in self.graph.nodes():
                ...
                node_trace['text'].append("@" + node + " is retweeted by "
                                          ...
                                          + str(self.graph.out_degree()[node])
                                          + " user(s)")
        ...
        fig =Figure(data=Data([edge_trace, node_trace]),
               layout=Layout(
                ...
                annotations=[
                    dict(text="Export to plot.ly to view the tweets and user information!",
                    showarrow=False,
        ...
        fname_div = 'div.html'
        with open(os.path.join(self.localSavePath, fname_div),"w") as f:
            f.write(div)
    ...
    def node_attributes(self):
        ...
        with open(os.path.join(self.localSavePath, fname_node),
                  "w", newline="") as f:
            writer = csv.writer(f)
    ...
    def edge_attributes(self):
        ...
        for edge in self.graph.edges():
            edge_attributes.append((edge,
                                    result['edge_betweenness_centrality'][edge],
                                    result['edge_load'][edge]))

        fname_edge = 'edge_attributes.csv'
        with open(os.path.join(self.localSavePath, fname_edge),
                  "w", newline="") as f:
            writer = csv.writer(f)
    ...
    def strong_components(self):
        ...
        with open(os.path.join(self.localSavePath, fname_strong),
                  "w", newline="") as f:
            writer = csv.writer(f)
    ...
    def weak_components(self):
        ...
        with open(os.path.join(self.localSavePath, fname_weak),
                  "w", newline="") as f:
            writer = csv.writer(f)
    ...
    def triads(self):
        ...
        with open(os.path.join(self.localSavePath,
                               fname_triads),"w",newline="") as f:
            writer = csv.writer(f)
...
def lambda_handler(event, context):
    ...
    # arranging the paths
    awsPath = os.path.join(event['s3FolderName'],
                           'NW', 'networkx', event['uid'])
    localSavePath = os.path.join('/tmp', event['s3FolderName'],
                                 'NW', 'networkx',
                                 event['uid'])
    ...
    # set network
    network = Network(awsPath, localSavePath, localReadPath,
                      event['remoteReadPath'], event['relations'])
    ...
    output['div'] = network.draw_graph(event['relations'], event['layout'])
